  ...
  const saveModule = async (data) => {
    try {
      if (moduleModal?.id) {
        ...
        await api.post(`/instructor/courses/${id}/modules`, { ...data, order_index: modules.length + 1 });
      }
      await fetchData();
  ...
  const saveLesson = async (data) => {
    try {
      ...
      if (lesson?.id) {
        ...
        await api.post(`/instructor/modules/${moduleId}/lessons`, { ...data, order_index: (mod?.lessons?.length || 0) + 1 });
      }
      await fetchData();
                      ...
                      <div key={lesson.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50/50 transition group">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-400 mt-0.5">
                            ...
                            {lesson.is_free_preview && <span className="bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded-full">Aperçu gratuit</span>}
                          </div>
                        </div>
