...
import {
  Save, ArrowLeft, Image, DollarSign, Clock,
  Globe, Tag, ToggleLeft, ToggleRight, Star,
  Upload, AlertCircle, CheckCircle, Loader
} from "lucide-react";

const LEVELS = ["beginner", "intermediate", "advanced"];
const LEVEL_LABELS = { beginner: "Débutant", intermediate: "Intermédiaire", advanced: "Avancé" };
const LANGUAGES = [{ value: "fr", label: "Français" }, { value: "en", label: "Anglais" }];
...
export default function CourseForm() {
  ...
  useEffect(() => {
    document.title = (isEdit ? "Modifier le cours" : "Créer un cours") + " — DevOpsAkademy";
    fetchCategories();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(typeof err === "string" ? err : "Une erreur est survenue. Vérifiez les champs.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const InputField = ({ label, name, type = "text", placeholder, required, help, ...rest }) => (
    <div>
      ...
      <input
        ...
        required={required}
        className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition bg-white"
        {...rest}
      />
  ...
  const TextareaField = ({ label, name, rows = 4, placeholder, help }) => (
    <div>
      ...
      <textarea
        ...
        rows={rows}
        placeholder={placeholder}
        className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition resize-none bg-white"
      />
      {help && <p className="text-xs text-gray-400 mt-1">{help}</p>}
    </div>
  );
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-white border border-gray-100 rounded-2xl shadow-soft p-6 space-y-5">
          ...
          {activeTab === "basic" && (
            <>
              <InputField label="Titre du cours" name="title" placeholder="Ex : Docker & Kubernetes de zéro à la production" required />
              <InputField label="Sous-titre" name="short_description" placeholder="Une phrase d'accroche percutante (max 120 caractères)" />
              <TextareaField label="Description complète" name="description" rows={5}
                placeholder="Décrivez en détail le contenu, les objectifs et la valeur apportée par votre cours…"
              />
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Catégorie</label>
                  <select name="category_id" value={form.category_id} onChange={handleChange}
                    className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 bg-white">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <InputField label="Durée estimée (heures)" name="duration_hours" type="number" placeholder="Ex : 12" min="0" step="0.5" />
              </div>
            </>
          )}

          {/* ── TAB : CONTENU & PRÉREQUIS ── */}
          {activeTab === "content" && (
            <>
              <TextareaField label="Ce que les apprenants vont apprendre" name="what_you_learn" rows={5}
          ...
          {activeTab === "media" && (
            <>
              <InputField label="URL de la miniature (thumbnail)" name="thumbnail_url" type="url"
                placeholder="https://…/image.jpg"
                help="Recommandé : 1280x720px, format JPG ou PNG"
              />
              {form.thumbnail_url && (
                <div className="rounded-xl overflow-hidden border border-gray-200 h-40 bg-gray-50">
                  <img src={form.thumbnail_url} alt="Preview" className="w-full h-full object-cover" onError={(e) => { e.target.style.display = "none"; }} />
                </div>
              )}
              <InputField label="URL de la vidéo de preview" name="video_preview_url" type="url"
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" name="price" value={form.price} onChange={handleChange}
                    ...
                    className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:bg-gray-100 disabled:cursor-not-allowed transition" />
                </div>
                <InputField label="Prix barré (FCFA)" name="original_price" type="number" placeholder="Ex : 25000" min="0"
                  help="Prix original affiché barré si promo" />
              </div>
            </>
          )}
          ...
          {activeTab === "settings" && (
            <div className="space-y-3">
              <Toggle name="is_published" label="Cours publié"
                desc="Le cours est visible et accessible aux apprenants inscrits" />
              <Toggle name="is_free" label="Cours gratuit"
                desc="L'accès est libre sans paiement. Le prix sera ignoré." />
              <Toggle name="is_featured" label="Cours mis en avant"
                desc="Affiché en priorité sur la page d'accueil et le catalogue" />
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-4 pt-2">
          ...
          <div className="flex gap-3">
            {/* Navigation tabs rapide */}
            {activeTab !== "settings" && (
              <button type="button" onClick={() => {
                const idx = TABS.findIndex(t => t.key === activeTab);
                setActiveTab(TABS[idx + 1]?.key || "settings");
              }} className="px-5 py-2.5 border border-primary text-primary rounded-xl hover:bg-primary/5 transition text-sm font-medium">
                Suivant →
              </button>
            )}
