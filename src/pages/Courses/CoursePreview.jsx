...
function ReviewCard({ review, isOwn, onEdit, onDelete }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border p-5 transition-all ${
      isOwn ? "border-indigo-200 bg-indigo-50/30" : "border-gray-100"
    }`}>
...
function ReviewForm({ courseId, existing, onSuccess, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (existing) {
        await api.put(`/courses/${courseId}/reviews`, { rating: Number(rating), comment: comment?.trim() || null });
      } else {
        try {
          await api.post(`/courses/${courseId}/reviews`, { rating: Number(rating), comment: comment?.trim() || null });
        } catch (postErr) {
          // 409 = avis déjà existant en BDD → fallback vers PUT
